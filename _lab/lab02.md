...
assigned: 2019-08-12 8:00:00.00-7
due: 2019-08-18 23:59:59.59-7
---
...
To start out, <b>write a comment with your name(s) and PERM number(s) in `{{page.num}}.py` (in general, you should write this on each of your submitted source files)</b>. Then write the line:

```
import pytest
```

Then, copy this function definition into your `{{page.num}}.py` file.
...
You are not finished with this lab quite yet. But, we want to encourage you to make a submission to Gradescope now anyway. Here is why:
...
3. It provides a backup copy of your work in case something goes wrong with your computer (yes, this can happen and you want to make sure there is a backup somewhere).
...
For this step, create a new file called `{{page.num}}_face.py` in your `~/cs8/{{page.num}}` directory. In general, remember to comment your name and PERM number (and your partner's name and PERM number if you're working in a group) on each .py file you work on.
...
Navigate to the Lab assignment "Lab02" on Gradescope and upload your `{{page.num}}.py` and `{{page.num}}_face.py`. Even though Gradescope will not auto-check your drawing, you must upload the `{{page.num}}_face.py` file to receive credit for this step.

For each submitted file `{{page.num}}.py` and `{{page.mum}}_face.py`, remember to write a comment with your (and your partner's if applicable) name and PERM number.

*Remember to add your partner to Group Members for this submission on Gradescope if applicable. At this point, if you worked in a pair, it is a good idea for both partners to log into Gradescope and see if you can see the uploaded files for Lab02.*
